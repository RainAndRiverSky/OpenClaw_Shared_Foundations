...
Deep Orbit's read-only obligations are binding:

- Deep Orbit must verify presentation index schema compatibility before
  rendering any index content.
- Deep Orbit must verify visibility classification before rendering any entry.
- Deep Orbit must verify that each entry's `evidenceState` is `canonical`
  before rendering it. Entries in `pending`, `ingested`, or `rejected` state
  must never be rendered.
- Deep Orbit must display freshness or staleness accurately and must never
  present stale data as current without an explicit stale indicator.
- Deep Orbit must preserve provenance references and must not strip them from
  rendered views.
- Deep Orbit must never treat a presentation index as canonical evidence.
- Deep Orbit must provide no direct mutation path into any resident's canonical
  evidence store.
- Deep Orbit must fail closed when eligibility or visibility cannot be
  established.

See [`schemas/DEEP_ORBIT_READ_CONTRACT.md`](schemas/DEEP_ORBIT_READ_CONTRACT.md)
for the complete binding specification.
...
Every canonical evidence artifact produced by a resident repository must carry
a stable, resident-namespaced identity in the following format:
...
- `residentId` is the resident's canonical identifier as declared in their
  ...
  account name.
- `artifactId` is a stable identifier assigned by the resident at creation
  time. It must be unique within the resident's namespace and must never be
  reused.

This namespace format prevents artifact identity collision across residents
without requiring a centralized artifact registry. No centralized registry is
mandated unless implementation evidence proves one is necessary.

The namespace rule applies to all canonical evidence artifacts referenced in
resident publications and presentation indexes.

## Canonical Evidence Distribution Flow

The canonical distribution flow governs how resident-owned evidence reaches a
presentation surface. Each step requires the prior step to have produced a
valid, schema-conformant output. No step is automatic.

```text
Canonical resident evidence   (governed by CANONICAL_EVIDENCE_CONTRACT)
        ↓
Resident publication          (governed by RESIDENT_PUBLICATION_CONTRACT)
        ↓
Presentation index            (governed by PRESENTATION_INDEX_CONTRACT)
        ↓
Deep Orbit read               (governed by DEEP_ORBIT_READ_CONTRACT)
```

**Canonical resident evidence** lives in the resident's own repository. Only
evidence with `evidenceState` of `canonical` may enter the flow.

**Resident publication** is produced by the resident. It assembles references
to canonical evidence artifacts and carries a distribution version. Only
canonical evidence may be referenced.

**Presentation index** is a derived, reproducible, disposable artifact
generated by Deep Orbit from a valid resident publication. It is not canonical.
It may be discarded and regenerated at any time.

**Deep Orbit read** is the presentation and rendering step. Deep Orbit consumes
the presentation index under binding read-only obligations. It must not write
to or alter canonical evidence.

The contracts governing each step are defined in
[`schemas/`](schemas/). Existing schemas and contracts are extended, not
replaced.
...
Governs how resident-owned canonical evidence is distributed to presentation
surfaces. These contracts are resident-agnostic and reusable across all
residents who conform to the Observatory.

- [`schemas/CANONICAL_EVIDENCE_CONTRACT.md`](schemas/CANONICAL_EVIDENCE_CONTRACT.md) —
  evidence admission states, transitions, and eligibility rules
- [`schemas/RESIDENT_PUBLICATION_CONTRACT.md`](schemas/RESIDENT_PUBLICATION_CONTRACT.md) —
  how residents publish canonical evidence references for distribution
- [`schemas/PRESENTATION_INDEX_CONTRACT.md`](schemas/PRESENTATION_INDEX_CONTRACT.md) —
  how derived presentation indexes are generated, kept fresh, and disposed
- [`schemas/DEEP_ORBIT_READ_CONTRACT.md`](schemas/DEEP_ORBIT_READ_CONTRACT.md) —
  binding read-only obligations for Deep Orbit when consuming a presentation index
